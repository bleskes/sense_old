...
ace.define('ace/mode/sense_json_highlight_rules', ['require', 'exports', 'module' , 'ace/lib/oop', 'ace/mode/text_highlight_rules'], function (require, exports, module) {
   ...
   var SenseJsonHighlightRules = function () {
      ...
      function addEOL(tokens, reg, nextIfEOF, normalNext) {
         if (typeof reg == "object") reg = reg.source;
         return [
            { token: tokens.concat(["whitespace"]), regex: reg + "(\\s*)$", next: nextIfEOF },
      ...
      this.$rules = {
         "start": [
            {token: "method", regex: "GET|PUT|POST|DELETE", next: "method_sep"}
            ,
            {
               token: "text",
               regex: ".+?"
         ...
         "method_sep": [
            {token: "whitespace", regex: "\\s+", next: "indices" }
         ],
         "indices": mergeTokens(
            addEOL(["url.scheme", "url.host", "url.slash"], /([^:]+:\/\/)([^?\/\s]*)(\/?)/, "jsonOrMethod"),
            addEOL(["url.endpoint"], /(_[^\/?]+)/, "jsonOrMethod", "urlRest"),
            addEOL(["url.index"], /([^\/?,]+)/, "jsonOrMethod"),
            addEOL(["url.comma"], /(,)/, "jsonOrMethod"),
            addEOL(["url.slash"], /(\/)/, "jsonOrMethod", "types"),
            addEOL(["url.questionmark"], /(\?)/, "jsonOrMethod", "urlParams")
         ),
         "types": mergeTokens(
            addEOL(["url.endpoint"], /(_[^\/?]+)/, "jsonOrMethod", "urlRest"),
            addEOL(["url.type"], /([^\/?,]+)/, "jsonOrMethod"),
            addEOL(["url.comma"], /(,)/, "jsonOrMethod"),
            addEOL(["url.slash"], /(\/)/, "jsonOrMethod", "id"),
            addEOL(["url.questionmark"], /(\?)/, "jsonOrMethod", "urlParams")
         ),
         "id": mergeTokens(
            addEOL(["url.endpoint"], /(_[^\/?]+)/, "jsonOrMethod", "urlRest"),
            addEOL(["url.id"], /([^\/?]+)/, "jsonOrMethod"),
            addEOL(["url.slash"], /(\/)/, "jsonOrMethod", "urlRest"),
            addEOL(["url.questionmark"], /(\?)/, "jsonOrMethod", "urlParams")
         ),
         "urlRest": mergeTokens(
            addEOL(["url.part"], /([^?\/]+)/, "jsonOrMethod"),
            addEOL(["url.slash"], /(\/)/, "jsonOrMethod"),
            addEOL(["url.questionmark"], /(\?)/, "jsonOrMethod", "urlParams")
         ),
         "urlParams": mergeTokens(
            addEOL(["url.param", "url.equal", "url.value"], /([^&=]+)(=)([^&]*)/, "jsonOrMethod"),
            addEOL(["url.param"], /([^&=]+)/, "jsonOrMethod"),
            addEOL(["url.amp"], /(&)/, "jsonOrMethod")
         ),
         "jsonOrMethod": [
            { token: "paren.lparen", regex: "{", next: "json" },
            { token: "punctuation.comma", regex: ",", next: "json" },
